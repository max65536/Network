...
int main(void) {
	...
	int flags=fcntl(socketfd,F_GETFL,0);
        fcntl(socketfd, F_SETFL, flags|O_NONBLOCK); //Setting fd to a nonblocking mode.
	...
	while(1){
        ...
        maxfdp=socketfd>fileno(stdin)?socketfd+1:fileno(stdin)+1;
           ...
           for(fd=0; fd<FD_SETSIZE; fd++ )
            {
                if(FD_ISSET(fd, &readfds))
                {
                    ...
                    else if(fd>=3) //client_fd
                    {
                        ioctl(fd, FIONREAD, &nread);//bytes in buf
                        printf("nread=%d\n",nread);
                    ...
                    else if(fd == 0)
                    {
                        ...
                        if(nread == 0)
                        {
                            close(fd);
                            FD_CLR(fd, &readfds);
                            printf("Removing keyboard on fd %d \n", fd);
                            continue;
                        }else
                        {
                            ctl=0;
